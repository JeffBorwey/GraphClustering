...
namespace NetMining.Graphs.Generator
{
    ...
    public class RandomGraphGenerator : IPointGraphGenerator
    {
        ...
        private int _expP = 1;
        private bool _useNormalizedProb = false;
        ...
        /// <summary>
        /// This sets the generative model used for creating a graph 
        /// </summary>
        /// <param name="val"></param>
        public void UseNormalizedProb(bool val)
        {
            _useNormalizedProb = val;
        }
        ...
        public LightWeightGraph GetNormalizedRandomGraph(DistanceMatrix d)
        {
            ...
            for (int i = 0; i < numNodes - 1; i++)
                for (int j = i + 1; j < numNodes; j++)
                    minDist = Math.Min(d[i, j], minDist);
            ...
            for (int i = 0; i < numNodes - 1; i++)
            {
                for (int j = i + 1; j < numNodes; j++)
                    ...
                    {
                        double addlProb = 1.0 / Math.Pow(d[i, j]/minDist, _expP);
